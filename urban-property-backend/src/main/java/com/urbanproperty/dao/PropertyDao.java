...
public interface PropertyDao extends JpaRepository<Property, Long> {
    ...
    @Query("SELECT new com.urbanproperty.dto.admin.PropertyStatusCountDto(p.status, COUNT(p)) FROM Property p GROUP BY p.status")
    List<PropertyStatusCountDto> countPropertiesByStatus();
    
    @Query("SELECT new com.urbanproperty.dto.admin.MonthlyPropertyStatsDto(MONTH(p.createdTime), COUNT(p)) " +
